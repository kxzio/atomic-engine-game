...
{
    char ch;
    // Получаем текущую длину текста через AnimatedChar
    std::string currentText = AnimatedChar(fmt, speed, &ch);

    // Отображаем текущий текст
    ImGui::Text("%s", currentText.c_str());

    // Если текст еще не завершен, добавляем прямоугольник
    if (currentText.length() < std::strlen(fmt))
    {
        ImVec2 cursorPos = ImGui::GetCursorScreenPos(); // Позиция курсора на экране
        ImVec2 textSize = ImGui::CalcTextSize(currentText.c_str()); // Размер отображаемого текста

        // Координаты прямоугольника
        ImVec2 rectStart = ImVec2(cursorPos.x + textSize.x - 3, cursorPos.y - textSize.y - 4);
        ImVec2 rectEnd = ImVec2(rectStart.x + 8.0f, rectStart.y + textSize.y);

        // Цвет прямоугольника
        ImU32 rectColor = IM_COL32(79, 255, 69, 255); 

        // Мигать прямоугольником
        if (static_cast<int>(ImGui::GetTime() * 6) % 2 == 0) // Мигание с частотой 0.5 сек
        {
            ImGui::GetWindowDrawList()->AddRectFilled(rectStart, rectEnd, rectColor);
        }
    }

    // Возвращаем true, если текст полностью отобразился
    return currentText.length() >= std::strlen(fmt);
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (flags & ImGuiButtonFlags_NoInput)
...
bool ImGui::Arrow(ImGuiDir dir, ImColor color)
{
    ImVec2 size = ImVec2(GetFrameHeight() / 3, GetFrameHeight() / 3);
    ...
    RenderArrow(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), color, dir);
...
bool ImGui::Checkmark(const char* label)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ImGui::SliderScalarForProgress(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = window->Size.x - 20;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 10));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
