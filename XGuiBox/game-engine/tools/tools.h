...
public:
    ...
    // Функция для вычисления поворота (cross product)
    float Cross(const Point& O, const Point& A, const Point& B) {
        return (A.x - O.x) * (B.y - O.y) - (A.y - O.y) * (B.x - O.x);
    }

    // Функция для ограничения значения в диапазоне [lo, hi]
    ...
    // Функция для вычисления интервалов перекрытия на окружности (в радианах) текущего круга от других кругов
    std::vector<std::pair<float, float>> GetOverlappingIntervals(const Circle& circle, const std::vector<Circle>& others) {
        std::vector<std::pair<float, float>> intervals;
        for (const auto& other : others) {
            // Если центры совпадают и радиусы равны, пропускаем (это тот же круг)
            ...
            // Если круги не пересекаются, пропускаем
            if (d > circle.radius + other.radius)
                continue;

            // Если текущий круг полностью внутри другого, то вся граница перекрыта
            if (d <= fabs(circle.radius - other.radius) && circle.radius <= other.radius) {
                intervals.push_back({ 0, 2 * 3.14 });
                continue;
            }

            // Определяем угол от центра текущего круга к центру другого круга
            float alpha = atan2f(dy, dx);
            // Вычисляем угол theta по теореме косинусов:
            ...
            // Нормализуем углы в диапазон [0, 2pi]
            start = fmodf(start + 2 * 3.14, 2 * 3.14);
            end = fmodf(end + 2 * 3.14, 2 * 3.14);

            // Если интервал «переполняет» 2pi, разбиваем его на два
            if (start > end) {
        ...
        // Сливаем интервалы (если они пересекаются или соприкасаются)
    ...
    // Вычисляем интервалы, в которых граница круга остаётся видимой (т.е. не перекрыта)
    ...
    void DrawCircleVisibleArcs(const Circle& circle, const std::vector<Circle>& allCircles, ImU32 color, float thickness) {
        ...
        // Вычисляем интервалы перекрытия и, на их основе, интервалы видимости
        auto overlapping = GetOverlappingIntervals(circle, others);
        auto visible = GetVisibleIntervals(overlapping);

        ImDrawList* draw_list = ImGui::GetBackgroundDrawList();
        // Если видимых интервалов нет, можно вообще не рисовать границу (или рисовать по своему усмотрению)
        if (visible.empty())
            return;

        // Для каждого видимого интервала отрисовываем дугу
        for (const auto& arc : visible) {
            float angleDiff = arc.second - arc.first;
            // Определяем число сегментов для сглаживания дуги
