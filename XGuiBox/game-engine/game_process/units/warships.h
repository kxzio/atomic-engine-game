...
public:
	void process_warships(int index, window_profiling window, std::vector <country_data>* countries, float animated_map_scale, int* hovered_id, ImVec2 cursor_pos, ImVec2 map_pos, int player_id, int function_count)
	{
		for (int i = 0; i < g_map.units.size(); i++)
		{
			...
			ImVec2 final_pos = ImVec2(pos.x + countries->at(g_map.units[i].owner_country_id).buildings[g_map.units[i].owner_building_id].pos.x * animated_map_scale + g_map.units[i].move_offset.x * animated_map_scale, pos.y + countries->at(g_map.units[i].owner_country_id).buildings[g_map.units[i].owner_building_id].pos.y * animated_map_scale + g_map.units[i].move_offset.y * animated_map_scale);
